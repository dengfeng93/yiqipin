...
| 信用 | 简化评价（到场？是/否 + 可选标签）、到场率展示、近期鸽子率（近7天）|
...
| 2026-06-25 | 开发环境启动修复：PostgreSQL 10.6 无 PostGIS/uuid-ossp 兼容适配（geography→text，ST_DWithin→Haversine，uuid由应用层生成）；JWT token空格清洗；准备时间改为开始+结束时间区间；路由顺序修复；user_profiles UPSERT修复；位置Web端固定坐标 |
